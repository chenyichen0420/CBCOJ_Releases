...
| :--- | :--- | :--- |
| 后端 | 支持 | 有代码实现，但是缺失多项测试且非维护版本，不提供 |
| 前端 | 支持 | 不支持 |

## 第一步：下载

从 Release v6.36.109 下载二进制文件。请确保下载的可执行文件与对应平台匹配。
...
*   第一行：账户数量（一个整数 $n$）。
*   后续 $n$ 行：每行存储一个账户的信息，依次为用户名和密码，以空格分隔。
*   相较于前一个版本，你不再需要手动配置用户 ID，用户 ID 按照配置顺序从 $1$ 开始分配。
...
`logi.txt`，`loge.txt`，`logw.txt` 是 `CBCOJ_Server.exe` 运行时生成的日志文件。在 `CBCOJ_Server.exe` 未运行时可以安全删除。
...
*   第一行：包含输入/输出文件名（不包含扩展名，对应的输入输出文件为 `*.in/out`，但是程序不应该读取或写入文件，评测端会重定向输入输出流），SPJ 所需的可执行文件名和子任务数量 $n$，以空格分隔。
    *   如果不需要SPJ，请用 `fc` 代替可执行文件名。
*   接下来有 $n$ 组子任务设置项，遵循如下格式：
    *   第一行先输入两个整数 $m,k$，表示这个子任务包含的测试点数量，以及通过这个子任务的得分。然后输入一个整数 $x$ 和 $x$ 个整数 $a_i$，表示当前的子任务依赖于子任务 $a_i$。$a_i$ 必须**小于**当前子任务的编号。子任务从 $0$ 开始编号。所有数字间用空格隔开。
    *   接下来 $m$ 行，每行包含数据点文件名（不包含扩展名，对应的输入输出文件存储为为 *.in/out），时间限制（ms）和空间限制（MiB），用空格隔开。
...
*   `adaccount` 数组中的每个元素应为 `{"username":"", "password":""}` 形式的对象，代表一对用户名和密码。
    *   `username`（字符串）：账户的用户名。
    *   `password`（字符串）：账户的密码。
*   `judgeServers`（数组）：用于评测的后端评测机的列表。
*   `judgeServers` 数组中的每个元素应为 `{"id":"", "name":"", "ip":"", "loginPort":num, "judgePort":num, "queryPort":num}` 形式的对象，代表一个后端的配置组。
    *   `id`（字符串）：这个后端配置组的编号，应当为 $[0,255]$ 中的一个整数的十进制字符串，不应有前导零。
    *   `ip`（字符串）：这个配置组的评测机的 IPv4 地址。
    *   `loginPort`（数字）：这个配置组的评测机的登录服务端口号。
    *   `judgePort`（数字）：这个配置组的评测机的提交服务端口号。
    *   `queryPort`（数字）：这个配置组的评测机的查询服务端口号。

在 `public/files` 文件夹中，您可以存放任何希望用户访问的文件，通过网址 `yourwebsite/files/文件名` 即可访问，但是默认配置不允许访问，需要手动访问管理员网页更改配置，将 `files` 从封禁文件夹中移除才可以。
...
（评测结果示意图：https://github.com/chenyichen0420/CBCOJ_Releases/blob/main/statics/Result_v5.png ）
...
CBCOJ 不支持 `testlib.h`，这看起来有些特别且可能不够便利，但有其设计考量。系统会通过命令行调用您的SPJ可执行文件，格式为：`spj.exe 输入文件路径 用户输出文件路径 答案文件路径`。
...
有的时候，如果你的前后端在同一个计算机上，并且在 `config.json` 中使用 `127.0.0.1` 作为评测机的 IPv4 地址，那么可以正常的登录、更新题目，但是无法获取评测结果。

该问题仅在 Win11 中出现过，而且未能稳定复现。不过一个可行的解决方案是将评测机的 IPv4 地址替换为本机在本地网络中的 IPv4 地址。
